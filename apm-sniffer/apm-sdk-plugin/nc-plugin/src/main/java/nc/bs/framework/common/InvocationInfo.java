...
public class InvocationInfo implements Externalizable {
	...
	private  java.util.HashMap<String, String>  hashmap=new java.util.HashMap<String, String>();
    private  Address address=null;
	
    public Address getAddress(){
    	return  address;
    }
    public  void setAddress(Address addressn){
...
//			Logger.error(new Exception("datasource is set null!"));
	...
	public void readExternal(ObjectInput in) throws IOException,
		...
		//boolean atServer = RuntimeEnv.getInstance().isThreadRunningInServer();
		...
		try {
			...
			hashmap=(java.util.HashMap<String,String>)in.readObject();
	...
	public void setCallPath(String callPath) {
		try {
			//callPath转为byte后暂不能超过1字节
			for (int len = callPath.getBytes("UTF-8").length; len > 127;) {
				callPath = callPath.substring(0, callPath.length() - 1);
				len = callPath.getBytes("UTF-8").length;
			}
